...
class sqlToMigrationCommand extends Command
{
	 ...
	 * @return void
	 */
	private function extractPrimaryKey(string $line, $current_table, $tables, array &$columns)
	{
		...
		if (stripos($line, 'primary key') === 0) {
			...
			if (preg_match('@\((.+)\)@', $line, $_matches)) {
				...
				if (isset($this->_auto_increment)) {
					$current_table = strlen($current_table) ? $current_table : end($tables);
	...
	public function _initialize()
	{
		...
		foreach ($lines as $line) {
			...
			if (!empty($columns)) {
				$this->extractPrimaryKey($line, $current_table, $tables, $columns);
