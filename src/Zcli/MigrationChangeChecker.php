    ...
    private function compareColumnsEntries(array $db_columns)
    {
        ...
        for ($i = 0; $i < count($db_columns); $i++) {
            ...
            if ($from_db != $from_migration) {
                ...
                if (!$bool) {
                    $bool = !$this->hasOnlyDiffColKey($diff_db, 'EXTRA') and !$this->hasOnlyDiffColKey($diff_mg, 'EXTRA');
                    $bool_2 = !$this->hasOnlyDiffColKey($diff_db, 'EXTRA') and $this->hasOnlyDiffColKey($diff_mg, 'EXTRA');

                    if ($bool_2) {
                        $this->statments[$i] = str_replace(')', ', true)', $this->statments[$i]);
                        $this->statments[$i] = str_replace('(, true)', '()', $this->statments[$i]);
                    }
