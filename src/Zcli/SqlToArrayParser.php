...
abstract class SqlToArrayParser
{
     ...
     * @return void
     */
    private static function captureModifierArg(string $item, array &$info, array $_matches)
    {
        // replace `` by ''
            ...
            if (stripos($str, $item) >= 0) {
                ...
                if ($item == 'null') {
                    ...
                    if (preg_match($_regex, $str, $_matches)) {
                        self::captureModifierArg($item, $info, $_matches);
                    }
                }
            }
        }
    ...
    /**
     * parse columns to default array format
     * 
